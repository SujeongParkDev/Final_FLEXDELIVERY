...
@Component
public class OwnerFileUploadUtil {
	//자료실에서 사용하는지, 상품 등록에서 사용하는지 구분하는 상수
	public static final int OWNER_MENU_TYPE=1;  //메뉴등록에 사용하는 값
	...
	private static final Logger logger
		=LoggerFactory.getLogger(OwnerFileUploadUtil.class);
	...
	public String getUploadPath(int type, HttpServletRequest request) {
		...
		String upPath="";
		if(type==OWNER_MENU_TYPE) {
			if(testGb.equals("test")) {
				upPath=fileUploadProps.getProperty("ownerMenu.upload.path.test");
			}else {
				upPath=fileUploadProps.getProperty("ownerMenu.upload.path");				
			}
		}

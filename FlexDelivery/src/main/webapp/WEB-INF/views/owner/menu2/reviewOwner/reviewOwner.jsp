										...
										<c:forEach var="map" items="${reviewList }">
											...
											<div class="card-content">
												<div class="card-body">
													...
													<p class="card-text">
														<span><fmt:formatDate value="${map['REVIEW_REGDATE']}" pattern="yyyy-MM-dd" /></span><br>
														<span style="color: purple;">${map['MENU_NAME']} + ${map['M_OPTION_NAME']}</span><br> <span>${map['REVIEW_RATING'] }점
														</span><br> <br> <br> 
														<span>${map['REVIEW_CONTENT']}</span>
													</p>
													<small class="text-muted"></small>
												</div>
												<!-- CEO comment start !! -->
												<c:if test="${!empty map['R_COMMENT_CONTENT']}">
													<div class="card-body backoground : #e9ecef7a;">
														<form id="frm2" method="post" action="<c:url value='/owner/menu2/reviewOwner/edit.do'/>">
															<input type="hidden" id="reviewNo" name="reviewNo" value="${map['REVIEW_NO'] }">
															<input type="hidden" id="storeNo" name="storeNo" value="1"> <!-- ${sessionScope.storeNo} 변경하기  -->
													
															<p class="comment-cont">
																사장님&nbsp;<span class="date">${map['R_COMMENT_REGDATE'] }</span>
															</p>
															<p class="card-text">
																${map['MEMBER_ID'] } 님, <br>${map['REVIEW_NO']}
															</p>
															<div class="result" id="resultDiv"></div>
															<div id="listDiv">
																${map['R_COMMENT_CONTENT'] } <br>
															<div class="button-group button-group-row align-right ">
																<button type="button" class="button small danger inGroup"
																	onclick="btDel(${map['REVIEW_NO']})">삭제</button>
																<button type="submit" class="button small secondary inGroup" id="btEdit" onclick="Edit_form(${map['REVIEW_NO']})">수정</button>
															</div>
															</div>
														</form>
													</div>
												...
												<c:if test="${empty map['R_COMMENT_CONTENT']}">
													<div class="card-body">
														<form id="frm1" method="post" action="<c:url value='/owner/menu2/reviewOwner/reviewOwnerWrite.do'/>">
															...
															<button class="button medium" type="submit" onclick="">댓글작성 </button>
														</form>
													</div>
													<hr>
												</c:if>
